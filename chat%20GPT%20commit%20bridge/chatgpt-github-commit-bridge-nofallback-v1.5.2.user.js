// ==UserScript==
// @name         ChatGPT → GitHub Commit Bridge (no-fallback) v1.5.2
// @namespace    https://github.com/BazookaJoe58/Torn-scripts
// @version      1.5.2
// @description  Per-block commit button (appears immediately, turns green when stable) + global fallback button. If direct commit is blocked, auto-branches and gives a PR link. Token fallback added so upgrades keep working.
// @author       BazookaJoe
// @license      MIT
...
// @downloadURL  https://github.com/BazookaJoe58/Torn-scripts/raw/refs/heads/main/chat%20GPT%20commit%20bridge/chatgpt-github-commit-bridge-nofallback-v1.5.2.user.js
// @updateURL    https://github.com/BazookaJoe58/Torn-scripts/raw/refs/heads/main/chat%20GPT%20commit%20bridge/chatgpt-github-commit-bridge-nofallback-v1.5.2.user.js
// ==/UserScript==

(() => {
  'use strict';

  // Singleton guard so only one instance runs
  if (window.__GHCB_SINGLETON__) return;
  window.__GHCB_SINGLETON__ = true;

  const QUIET_MS = 900; // time without code changes before "Ready"
  const GLOBAL_BTN_ID = 'ghcb-global-btn';
  const TOKEN_KEYS = ['gh_token', 'gh_token_global'];
  ...
  GM_addStyle(`
    ...
    .ghcb-btn.ready{ background:#28a745; border-color:#1f7a34; cursor:pointer; }
    ...
    .ghcb-toast{
      ...
      font: 13px/1.45 system-ui,Segoe UI,Arial; white-space:normal;
    }
  ...
  // ---------- Token helpers (GM storage + localStorage fallback) ----------
  function getToken() {
    for (const k of TOKEN_KEYS) {
      try { const v = GM_getValue(k, ''); if (v) return v; } catch {}
    }
    for (const k of TOKEN_KEYS) {
  ...
  function saveToken(v) {
    ...
    try { TOKEN_KEYS.forEach(k => GM_setValue(k, val)); } catch {}
    try { TOKEN_KEYS.forEach(k => localStorage.setItem(k, val)); } catch {}
  }

  GM_registerMenuCommand('Set GitHub Token', () => {
    const cur = getToken();
    const v = prompt('Paste a GitHub fine-grained token (repo:contents:write):', cur || '');
    if (v !== null) { saveToken(v); toast('🔐 <b>Token saved</b>.'); }
  });
  ...
  // ---------- GitHub commit logic (direct → branch/PR fallback) ----------
  ...
  async function commitToGitHub({owner, repo, branch, path, content, message}) {
    ...
    if (!token) throw new Error('Missing GitHub token. Use Tampermonkey menu → “Set GitHub Token”.');
    ...
    if (res.status >= 200 && res.status < 300) return { mode: 'direct', branch, res };
  ...
  // ---------- Wire per-block button (watch only <code> for stability) ----------
  ...
  function ensureGlobalButton() {
    ...
    b.addEventListener('click', async () => {
      ...
      // choose largest block by text length
      ...
      if (!code.trim()) { toast('No code detected in the selected block.'); return; }
      const target = detectTargetFromHeader(code);
