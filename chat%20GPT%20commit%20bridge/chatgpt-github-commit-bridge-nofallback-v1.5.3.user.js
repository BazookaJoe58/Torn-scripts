// ==UserScript==
// @name         ChatGPT → GitHub Commit Bridge (no-fallback) v1.5.3
// @namespace    https://github.com/BazookaJoe58/Torn-scripts
// @version      1.5.3
// @description  Per-block commit button (appears immediately, turns green when stable) + global fallback button. If direct commit is blocked, auto-branches and gives a PR link. Token fallback so upgrades keep working.
// @author       BazookaJoe
// @license      MIT
...
// @downloadURL  https://github.com/BazookaJoe58/Torn-scripts/raw/refs/heads/main/chat%20GPT%20commit%20bridge/chatgpt-github-commit-bridge-nofallback-v1.5.3.user.js
// @updateURL    https://github.com/BazookaJoe58/Torn-scripts/raw/refs/heads/main/chat%20GPT%20commit%20bridge/chatgpt-github-commit-bridge-nofallback-v1.5.3.user.js
// ==/UserScript==
...
(() => {
  ...
  const QUIET_MS = 900; // ms with no code changes before "Ready"
  ...
  // ---------- Token helpers (GM + localStorage fallback) ----------
  function getToken() {
  ...
  async function commitToGitHub({owner, repo, branch, path, content, message}) {
    ...
    if (res.status === 403 || res.status === 409) {
      ...
      if (commitRes.status >= 200 && commitRes < 300) {
