// ==UserScript==
// @name         ChatGPT → GitHub Commit Bridge (no-fallback) v1.5.4
// @namespace    https://github.com/BazookaJoe58/Torn-scripts
// @version      1.5.4
// @description  Per-block commit button (appears immediately, turns green when stable) + global fallback. If direct commit is blocked, auto-branches and gives a PR link. Token fallback so upgrades keep working across reinstalls.
// @author       BazookaJoe
// @license      MIT
...
(() => {
  ...
  // Singleton so only one instance runs
  if (window.__GHCB_SINGLETON__) return;
  window.__GHCB_SINGLETON__ = true;

  const QUIET_MS = 900;                 // ms with no code changes before "Ready"
  const GLOBAL_BTN_ID = 'ghcb-global';  // global fallback button id
  const TOKEN_KEYS = ['gh_token', 'gh_token_global']; // GM + localStorage keys
