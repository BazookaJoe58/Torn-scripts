...
// @downloadURL  https://raw.githubusercontent.com/BazookaJoe58/Torn-scripts/main/userscripts/torn-market-view-green-button.v0.1.4.user.js
// @updateURL    https://raw.githubusercontent.com/BazookaJoe58/Torn-scripts/main/userscripts/torn-market-view-green-button.v0.1.4.user.js
// ==/UserScript==

(function () {
  'use strict';

  // Only run on the View Listing tab
  if (!location.href.includes('page.php?sid=ItemMarket') || !location.href.includes('#/viewListing')) return;
  ...
  const scan = (ctx = document) => {
    ctx
      ...
      .forEach((priceWrapper) => {
        priceWrapper.closest('[class*=itemRowWrapper___]')?.classList.add('silm-gb-processed');
  ...
  const mo = new MutationObserver((muts) => {
    for (const m of muts) {
      ...
      if (cls.includes('viewListingWrapper___') || cls.includes('priceInputWrapper___')) scan(t);
    }
  });
