// ==UserScript==
// @name         Torn Item Market — Quarter-Overlay Fill Max on Buy (Right, No-Scroll)
// @namespace    https://torn.city/
// @version      2.3.8
// @description  Rightmost 25% "Fill Max" overlay on Torn's native Buy/Confirm; fills max, then drops behind so native Buy is clickable. No scrolling, no bypasses.
// @author       Baz
...
(function () {
  ...
  function findNativeBuyButton(row){
    ...
    const btns=Array.from(li.querySelectorAll(SEL.buyButtons)).filter(b=>{
      ...
      if (!txt) return false;
      if (/show\s*buy/i.test(txt)) return false; // skip the "Show Buy" toggle
      return /buy|confirm|purchase/.test(txt);
    });
    // Prefer visible
    const vis = btns.find(b=>b.getBoundingClientRect().width>0 && b.getBoundingClientRect().height>0);
  ...
  async function placeOverlay(row){
    ...
    // Anchor overlay to the native button
    ...
    overlay.addEventListener('click', async (e)=>{
      ...
      // Drop behind so the native Buy is fully clickable
      overlay.classList.add('im-done');
      focusNoScroll(native);
    }, {capture:true});
  }
