...
(function () {
  ...
  // Force start minimised each page load (you can toggle open via right tab)
  const FORCE_MIN_ON_LOAD = true;

  const STORAGE = {
    key: 'tcos_api_key_v5',
    keyV4: 'tcos_api_key_v4', // migrate
  ...
  GM_addStyle(`
    ...
    #tcos-minitab{
      ...
      padding:10px 6px; background:rgba(0,0,0,0.85); color:#fff;
      border-top-left-radius:10px; border-bottom-left-radius:10px;
      border:1px solid #444; border-right:0; z-index:2147483645;
      cursor:pointer; font-weight:800; font-size:12px; writing-mode:vertical-rl;
      text-orientation:mixed; display:none; user-select:none;
    }

    /* Draggable pill (starts docked under mini-tab) */
    .tcos-pill{
      position:fixed; right:0; top:calc(40% + 70px); transform:translateY(-50%);
      min-width:140px; padding:8px 10px; border-radius:10px 0 0 10px;
  ...
  function ensureUI() {
    ...
    miniTab = qs('tcos-minitab') || (()=>{const t=document.createElement('div');t.id='tcos-minitab';t.textContent='Sentinel';safeAppend(t);return t;})();
    pill = document.querySelector('.tcos-pill') || (()=>{const s=document.createElement('div');s.className='tcos-pill';s.innerHTML=`
      ...
      <div><span>OC</span><span id="pill-oc">--</span></div>`; safeAppend(s); return s;})();
    ...
    if (!miniTab.dataset.bound){ miniTab.addEventListener('click',()=>setMinimised(false)); miniTab.dataset.bound='1'; }
  ...
  // Persist
  async function migrateKey(){ const v5=await GM_getValue(STORAGE.key,''); if(v5) return v5;
    const v4=await GM_getValue(STORAGE.keyV4,''); const v3=await GM_getValue(STORAGE.keyV3,''); const m=v4||v3||'';
    if(m) await GM_setValue(STORAGE.key,m); return m; }
  async function loadPersisted() {
    ...
    // START MINIMISED every load unless you open the tab this load
    let min = true;
  ...
  async function refreshFromServer() {
    // Always grab fresh key if empty (covers "typed but not saved yet" edge)
  ...
  async function init(){
    ...
    // Start minimised by default on each load (can be opened via tab)
    ...
    const mo=new MutationObserver(()=>{ if(!qs('tcos-panel')||!qs('tcos-minitab')||!document.querySelector('.tcos-pill')||!qs('tcos-overlay')||!qs('tcos-modal-wrap')){ ensureUI(); makePillDraggable(); } });
